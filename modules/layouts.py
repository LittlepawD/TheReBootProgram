...
def calendar_box(text="", bt_color=None, enabled=True):
    ...
    return sg.Btn(text, key=key, size=(3, 1), button_color=bt_color, font="courier 13", pad=(0, 0),
...
def create_journal_layout():
    ...
    day = date.today()
    journal_text = 'Today is: ' + str(day) + '. Write down how your day is going or what you want to achieve.'
    menu_layout: list = [['File', [file_new, file_open, file_save, 'Save As', '---', 'Exit']],
                         ['Help', ['About']]]
    layout = [
        [sg.Menu(menu_layout)],
        [sg.Text('> New entry <', font=('Consolas', 10), size=(600, 1), key='_INFO_')],
        [sg.Text(journal_text)],
        [sg.Multiline(font=('Consolas', 12), size=(600, 500), key='_BODY_')],
        [sg.Button('Exit')]]
